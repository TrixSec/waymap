...
def parse_union_tests_from_xml(file_path: str = None) -> List[Dict[str, str]]:
    """Parse union query-based SQLi test cases from XML."""
    if file_path is None:
        file_path = os.path.join(config.DATA_DIR, "union_query.xml")
...
def union_based_sqli(url: str, test: Dict[str, str], thread_count: int) -> bool:
...
def process_urls(urls: List[str], thread_count: int) -> None:
    ...
    tests = parse_union_tests_from_xml()
    if not tests:
    ...
    def check_url_test(url_test_tuple):
        ...
        if stop_scan.is_set(): return False
        return union_based_sqli(url, test, thread_count)
